            ...
            try:
                ...
                if date_xml_data:
                    ...
                    Validation_Output.append( helper_fuc(sequence_number, 'Asset_Level', f'Validate Asset Thumbnail Aspect Ratio in all 7 days', f'Asset Thumbnail should be 1920X1080 resolution in all 7 days', 'Failed', f'Asset Thumbnail are having in-correct-thumbnail aspect ratio. But, expected should be 16:9 aspect-ratio', ','.join(map(str, results[12]))) if results[12] else
                                              helper_fuc(sequence_number, 'Asset_Level', f'Validate Asset Thumbnail Aspect Ratio in all 7 days', f'Asset Thumbnail should be 1920X1080 resolution in all 7 days', 'Not Tested', f'Asset Thumbnail not available for some assets', ','.join(map(str, results[2]))) if results[2] else
                                              helper_fuc(sequence_number, 'Asset_Level', f'Validate Asset Thumbnail Aspect Ratio in all 7 days', f'Asset Thumbnail should be 1920X1080 resolution in all 7 days', 'Not Tested', f'Main Programme Field itself not available', ','.join(map(str, results[0]))) if results[0] else
                                              helper_fuc(sequence_number, 'Asset_Level', f'Validate Asset Thumbnail Aspect Ratio in all 7 days', f'Asset Thumbnail should be 1920X1080 resolution in all 7 days', 'Not Tested', f'Icon Tag not available for some assets', ','.join(map(str, results[1]))) if results[1] else
                    ...
                    Validation_Output.append(helper_fuc(sequence_number, 'Asset_Level', f'Validate Rating Value as per Samsung standard in all 7 days', f'Rating Source Value present in Samsung_Supported_Rating_Value_List in all 7 days', 'Failed', f'For some assets, having in-correct-cat rating value are not included in Samsung_Supported_Category_List', ','.join(map(str, results[5]))) if results[5] else
